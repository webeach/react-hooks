...
- **Параметры**
  ...
  - `delayMs` _(опционально)_ — дефолтная длительность таймера. Используется при запуске без указания override‑значения.
...
```tsx
import { useTimeoutExtended } from '@webeach/react-hooks';
